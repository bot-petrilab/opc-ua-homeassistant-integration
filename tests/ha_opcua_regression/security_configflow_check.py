    ...
    async with async_playwright() as p:
        ...
        async def api(m, path, data=None):
            raw = await page.evaluate(
                """async (a)=>{const ha=document.querySelector('home-assistant');try{return JSON.stringify({ok:true,out:await ha.hass.callApi(a.m,a.p,a.d||undefined)});}catch(e){return JSON.stringify({ok:false,err:e});}}""",
                {"m": m, "p": path, "d": data},
            )
            obj = json.loads(raw)
            if not obj["ok"]:
        ...
        for e in [
            ...
            and x.get("title") in ["SEC CFG TEST", "SEC CFG TEST 2"]
        ]:
            try:
        ...
        ok_missing = (
            r1.get("type") == "form"
            and "client_cert_path" in (r1.get("errors") or {})
            and "client_key_path" in (r1.get("errors") or {})
        )
        ...
        r2 = await api(
            "POST",
            f"config/config_entries/flow/{fid2}",
            {
