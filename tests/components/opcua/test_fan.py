...
async def test_fan_turn_on_accepts_preset_mode_and_sets_speed(coordinator_bool) -> None:
    ...
    assert coordinator_bool.manager.writes[0] == ("ns=2;s=EntityMatrix.Operation.Running", True)
...
async def test_fan_turn_off_writes_false(coordinator_bool) -> None:
    ...
    assert coordinator_bool.manager.writes[-1] == ("ns=2;s=EntityMatrix.Operation.Running", False)
