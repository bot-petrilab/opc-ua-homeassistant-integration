...
async def main() -> None:
    ...
    try:
        ...
        service_name = f"Basic256 OPC UA Simulator {host_ip}.{service_type}"
        service_info = ServiceInfo(
            ...
            server=f"opcua-basic256-{host_ip.replace('.', '-')}.local.",
        )
    ...
    try:
        async with server:
            print(f"OPC UA Basic256 simulator running at opc.tcp://0.0.0.0:{PORT}")
