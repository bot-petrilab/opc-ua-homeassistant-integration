...
async def main() -> None:
    ...
    light_color_temp = await _add_var(
        lighting,
    ...
    await _add_var(
        device_panel,
        f"ns={idx};s=Home.Devices.Panel01.Manufacturer",
        "Manufacturer",
        "Petri Automation",
        False,
    )
    await _add_var(
        device_panel,
        f"ns={idx};s=Home.Devices.Panel01.Model",
        "Model",
        "PA-LightPanel",
        False,
    )
    await _add_var(
        device_panel,
        f"ns={idx};s=Home.Devices.Panel01.SerialNumber",
        "SerialNumber",
        "PA-PANEL-0001",
        False,
    )
    panel_lights = await device_panel.add_object(
        f"ns={idx};s=Home.Devices.Panel01.Lights", "Lights"
    )

    device_rgb = await home_devices.add_object(
        f"ns={idx};s=Home.Devices.RgbController01",
        "RGB Controller 01",
        objecttype=device_type,
    )
    await _add_var(
        device_rgb,
        f"ns={idx};s=Home.Devices.RgbController01.Manufacturer",
        "Manufacturer",
        "Petri Automation",
        False,
    )
    await _add_var(
        device_rgb,
        f"ns={idx};s=Home.Devices.RgbController01.Model",
        "Model",
        "PA-RGBWW",
        False,
    )
    await _add_var(
        device_rgb,
        f"ns={idx};s=Home.Devices.RgbController01.SerialNumber",
        "SerialNumber",
        "PA-RGB-0001",
        False,
    )
    rgb_lights = await device_rgb.add_object(
        f"ns={idx};s=Home.Devices.RgbController01.Lights", "Lights"
    )
    ...
    await _add_var(
        light_obj_main, f"ns={idx};s=Home.Lights.MatrixMain.State", "State", False, True
    )
    await _add_var(
        light_obj_main,
        f"ns={idx};s=Home.Lights.MatrixMain.Brightness",
        "Brightness",
        128,
        True,
    )
    ...
    await _add_var(
        light_obj_corridor,
        f"ns={idx};s=Home.Lights.Corridor.State",
        "State",
        True,
        True,
    )
    ...
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.State", "State", False, True
    )
    await _add_var(
        light_obj_full,
    ...
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.Hue", "Hue", 210.0, True
    )
    await _add_var(
        light_obj_full,
    ...
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.R", "R", 120, True
    )
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.G", "G", 90, True
    )
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.B", "B", 255, True
    )
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.RGBW_R", "RGBW_R", 120, True
    )
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.RGBW_G", "RGBW_G", 90, True
    )
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.RGBW_B", "RGBW_B", 255, True
    )
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.RGBW_W", "RGBW_W", 40, True
    )
    await _add_var(
        light_obj_full,
        f"ns={idx};s=Home.Lights.RainbowPro.RGBWW_R",
        "RGBWW_R",
        120,
        True,
    )
    await _add_var(
        light_obj_full,
        f"ns={idx};s=Home.Lights.RainbowPro.RGBWW_G",
        "RGBWW_G",
        90,
        True,
    )
    await _add_var(
        light_obj_full,
        f"ns={idx};s=Home.Lights.RainbowPro.RGBWW_B",
        "RGBWW_B",
        255,
        True,
    )
    await _add_var(
        light_obj_full,
        f"ns={idx};s=Home.Lights.RainbowPro.RGBWW_CW",
        "RGBWW_CW",
        45,
        True,
    )
    await _add_var(
        light_obj_full,
        f"ns={idx};s=Home.Lights.RainbowPro.RGBWW_WW",
        "RGBWW_WW",
        55,
        True,
    )
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.White", "White", 80, True
    )
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.X", "X", 0.31, True
    )
    await _add_var(
        light_obj_full, f"ns={idx};s=Home.Lights.RainbowPro.Y", "Y", 0.33, True
    )
    await _add_var(
        light_obj_full,
        f"ns={idx};s=Home.Lights.RainbowPro.Effect",
        "Effect",
        "off",
        True,
    )
    ...
    await _add_var(
        light_obj_full,
        f"ns={idx};s=Home.Lights.RainbowPro.Flash",
        "Flash",
        "short",
        True,
    )

    weather_condition = await _add_var(
        weather,
    ...
    weather_message = await _add_var(
        diagnostics,
    ...
    try:
        async with server:
            ...
            print(
                "Entity matrix runs in static mode: values stay unchanged unless a client writes them."
            )
            while True:
